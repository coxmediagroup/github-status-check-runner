...
export const getDefaultCommitHash = (): string => {
  ...
  const execSyncOpts: child_process.ExecSyncOptions = { encoding: 'utf8', stdio: ['inherit', 'pipe', 'ignore'] };

  try {
    gitHead = child_process.execSync('git rev-parse HEAD', execSyncOpts).toString().trim();
  } catch (error) {
